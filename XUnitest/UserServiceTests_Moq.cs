...
namespace Xunitest
{
    ...
    {
        // Uses Moq to mock IRepository, isolating UserService.
        [Fact]
        public void GetUserStatus_ActiveUser_ReturnsActive()
        {
            // Arrange
            ...
            // Act
            string result = userService.GetUserStatus(1);

            // Assert
            Assert.Equal(expected, result);
            mockRepository.Verify(repo => repo.IsUserActive(1), Times.Once());
        }

        [Fact]
        public void GetUserStatus_InactiveUser_ReturnsInactive()
        {
            // Arrange
            var mockRepository = new Mock<IRepository>();
            mockRepository.Setup(repo => repo.IsUserActive(2)).Returns(false);
            var userService = new UserService(mockRepository.Object);
            string expected = "Inactive";

            // Act
            string result = userService.GetUserStatus(2);

            // Assert
            Assert.Equal(expected, result);
            mockRepository.Verify(repo => repo.IsUserActive(2), Times.Once());
        }
